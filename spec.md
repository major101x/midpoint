...
Target network: Coston2 (chain ID 114) · Solo build · 15 days (2026-07-30 → 2026-08-14)
...
FAssets agents, market makers, and treasuries moving size in FXRP. Concretely: anyone whose order is large enough relative to available liquidity that revealing it costs them money. This is a small user set with an acute, quantifiable problem — the right shape for a hackathon product, and the right shape for something with a life afterwards.
...
| **Attestation** — the TEE proves which code it runs | Need to trust the operator | It's a centralised exchange with extra steps |
...
4. fills net to zero — total bought equals total sold.
...
`Settlement.sol` independently reads XRP/USD from FTSO v2 and reverts if the clearing price deviates by more than `BAND_BPS` (initially 200 = 2%). The TEE's discretion is bounded by an oracle it does not control and cannot influence. A malicious or buggy engine cannot print an arbitrary price.
...
- Feeds return `(value, decimals, timestamp)` — integers plus a scale exponent, no floats on-chain.
...
**Encrypted order** (client → TEE, never on-chain in plaintext):
...
**Settlement payload** (TEE → relayer → chain):
...
|---|---|---|
| MEV bot / mempool observer | Reads all transactions | Sees ciphertext and deposit sizes. Cannot recover side, price or size. Cannot gain from ordering — uniform price. |
...
| Malicious TEE code | Ships a rigged engine | Image hash on-chain ≠ hash of published source; independently reproducible builds expose it. Clearing price still bounded by the FTSO band. |
...
- Uniform-price clearing, ~10–20 orders per batch
...
- Cross-TEE replication / failover
...
| 7 | Aug 5 | FCE: clearing algorithm, unit-tested off-chain | Auction tests pass, incl. pro-rata and empty-book |
...
| 13 | Aug 11 | Buffer. Polish, attestation docs, deploy addresses, error paths | — |
| 14 | Aug 12 | Record video, write README + submission | Video under 4 min, links live |
| 15 | Aug 13 | **Submit.** Post in the Flare Telegram, get 3–5 people to try it | Submitted, 24h before deadline |
...
| Clarity + future potential | This document; the stated cut lines; the PMW→XRPL roadmap |
